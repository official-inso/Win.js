...
let Win = {
      ...
      if(response != undefined || win_has){
        ...
        if(!win_has) {
          ...
          if(content == undefined) {
            ...
            yt = await Win.service.output(gth.res, gth.name, gth.params);
          } else {
            yt = Win.service.outputLocal(content, gth.name, gth.params);
          }
          ...
          if(content != undefined){
            let w = Win.get(name);
            if(w != false) {
              let wContent = w.querySelector('.windows-window-content-div > content')
              wContent.innerHTML = content;
            }
  ...
  /**
   * The method returns the window by its ID
   * @param {String} name Window ID
   * @returns Return false if window not found or DOM object if window exists
   */
  get: (name) => {
    let wind = document.querySelector(`#${name}`);
  ...
  service: {
    ...
    outputLocal: async (response, name, params) => {
